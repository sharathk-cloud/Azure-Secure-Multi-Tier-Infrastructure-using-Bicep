...
resource sqlServer 'Microsoft.Sql/servers@2022-05-01-preview' = {
  ...
  properties: {
    ...
    // Public access is on but restricted — only the App subnet
    // VNet rule below can reach this server. All other internet
    // traffic is denied by the VNet rule.
    // Note: publicNetworkAccess: Disabled conflicts with VNet rules.
    // Private Endpoint is the production alternative.
...
// ── VNet Rule ─────────────────────────────────────────────────
// Restricts SQL Server access to the App subnet.
// Requires the Microsoft.Sql service endpoint on the App subnet
// (added to vnet.bicep in Lab 12).
